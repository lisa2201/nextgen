import { Component, OnInit, ViewEncapsulation, OnDestroy, Inject, Renderer2, ElementRef, ViewChild } from '@angular/core';
...
import { BookingAddConfigComponent } from 'app/main/modules/modules-shared/booking/dialogs/add-config/add-config.component';
...
export interface BookingDayTime {
    id: string;
    index: number;
    day_name: string;
    disable: boolean;
...
export class ChildAddBookingComponent implements OnInit, OnDestroy {
    ...
    )
    {
        ...
        this.dialogTitle = 'Create New Booking';
    ...
    onChanges(): void
    {
        ...
        this.bookingForm
            ...
            .subscribe(value =>
            {
                ...
                this.nonLinkedRoomsFound = this.child.rooms.filter(r => r.id === value).length < 1;
            });
    ...
    {
        return this.child.rooms.filter(r => r.id === room.id).length > 0;
    }
     ...
     */
    openConfigDialog(e: MouseEvent, item: BookingDayTime, index: number = null): void
    {
        e.preventDefault();
        ...
        this.dialogRef = this._matDialog
            .open(BookingAddConfigComponent,
            {
                panelClass: 'child-booking-add-config',
                closeOnNavigation: true,
                disableClose: true,
                autoFocus: false,
                data: {
                    action: _.isNull(index) ? AppConst.modalActionTypes.NEW : AppConst.modalActionTypes.EDIT,
                    rooms: this.rooms,
                    fees: this.fees,
                    index: index,
                    response: item.value
        ...
        this.dialogRef
            ...
            {   
                ...
                if (_.isNull(index))
                {
                    this.bookingDateTimeMap[item.index]
                        .value
                        .push({
                            room: data['room'],
                            fee: data['fee'],
                            start: (data['fee'].hasSession() && _.isNull(data['session'])) ? data['fee'].sessionStart : _.head(data['session']),
                            end: (data['fee'].hasSession() && _.isNull(data['session'])) ? data['fee'].sessionEnd : _.last(data['session'])
                        });
                }
                else
                {
                ...
                this.bookingDateTimeMap[item.index].value = _.sortBy(this.bookingDateTimeMap[item.index].value, (i: BookingTime) => i.start);
            });
    }
     ...
     * @param {number} index
     */
    removeSlot(e: MouseEvent, day: any, index: number): void
    {
        e.preventDefault();
            ...
            .confirm(
                {
                    ...
                    nzOkText: 'Yes',
                    nzOkType: 'danger',
                    nzOnOk: () => setTimeout(() => _.remove(this.bookingDateTimeMap[day.index].value, (i, idx) => idx === index), 250)
                }
            );
    }
    ...
    onFormSubmit(e: MouseEvent): void
    {
        ...
        console.log(this.getSelectedPreviewSlotItems());
        ...
        this._logger.debug('[submit child booking]', sendObj);

        this.buttonSaveLoader = true;

        this._bookingService
            .storeBooking(sendObj)
            .pipe(
                takeUntil(this._unsubscribeAll),
