...
export class CommonService {
     ...
     * @returns {string}
     */
    getS3FullLink(path: string): string 
    {
        return `${AppConst.s3Prefix}${path}`;
    }

    getS3FullLinkforProfileImage(path: string): string 
    {
        return `${AppConst.profileImageS3Prefix}${path}`;
    }

    extractS3FileName(key: string): string 
    {
        return _.replace(_.last(_.split(key,'/')), /\b[0-9a-f]{8}\b-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-\b[0-9a-f]{12}-\b/g,'');
    }
    ...
    getChildrenWithoutPrimaryPayer(): Observable<any> {

        if (this._authService.isAuthenticated() && this._authService.isAdministrative()) {

            return this._httpClient.get<any>(`${AppConst.apiBaseUrl}/${AppConst.urlPrefix.APP}/finance-children-without-payer`)
            .pipe(
                delay(2000),
                shareReplay(),
                map((response) => {
                    ...
                    if (response.data) {
                        const children = _.map(response.data, (val: any, idx: number) => new Child(val, idx));
                        this.onChildListChanged.next(children);
                        return children;
                    } else {
                        return [];
                    }
    ...
    getAllImmunisationType(): Promise<any>
    {
        return new Promise((resolve, reject) => 
        {
            ...
            .pipe(
                map(response => response.data.map((i: any, idx: number) => new Immunisation(i, idx))),
                shareReplay()
            )
            .subscribe(
                (response: any) => resolve(response),
    ...
    getAllImmunisationTracking(): Promise<any>
    {
        return new Promise((resolve, reject) => 
        {
            ...
            .pipe(
                map(response => response.data.map((i: any, idx: number) => new ImmunisationTracker(i, idx))),
                shareReplay()
            )
            .subscribe(
                (response: any) => resolve(response),
