...
export class AuthService {
    ...
    {
        return this._httpClient
            .post<any>(`${AppConst.apiBaseUrl}/verify_kinder_connect_access`, { token: token })
            .pipe(
                map(response => response.data),
                tap(response => this.updateTokens(response)),
                tap(response => this.setKinderConnectPath(response)),
                tap(response => this.resolveDefaultPath(response.__iED)),
                shareReplay()
            );
    }
    ...
    {
        return this._httpClient
            .post<any>(`${AppConst.apiBaseUrl}/forgot-password`, { email: _email })
            .pipe(
    ...
    {
        return this._httpClient
            .post<any>(`${AppConst.apiBaseUrl}/reset-user-password`, data)
            .pipe(
                map(response => response.message),
                shareReplay()
            );
     ...
     * @returns {Observable<any>}
     */
    loginToKinderConnect(): Observable<any>
    {
        const params = new HttpParams().set('domain', _.trim(UrlHelper.extractTenantNameFromUrl(location.host)));

        return this._httpClient
            .get<any>(`${AppConst.apiBaseUrl}/access-kinder-connect`, { params })
            .pipe(
                map(response => response.data),
                shareReplay()
            );
    ...
    {
        this._logger.debug('[auth object size]', CommonHelper.getSizeOf(response, true));
        ...
        // resolve kinder connect path
        this.resolveKinderConnectPath();
    }
    ...
    getKinderConnectPath(): string
    {
        try
        {
            return this._localStorage.retrieve(AppConst.auth.kinderConnectPath);
        }
        catch(error)
        {
            return null;
    ...
    removeKinderConnectPath(): void
    {
        try
        {
            this._localStorage.clear(AppConst.auth.kinderConnectPath);
        }
        catch(error)
        {
            CommonHelper.errorLog(error);
    ...
    resolvePaymentMethodPath(state: RouterStateSnapshot): boolean 
    {
        if (this._domain === AppConst.appStart.SITE_MANAGER.NAME) 
        {
            if (state.url === `${AppConst.appStart.PAYMENT.BASE_URL}/${AppConst.appStart.PAYMENT.PAYMENT_STARTUP}`) 
            {
                return true;
            } 
            else 
            {
                this._logger.debug('No payment method found. Redirect to payment startup page.');
                
                setTimeout(() => this._router.navigate([AppConst.appStart.PAYMENT.BASE_URL, AppConst.appStart.PAYMENT.PAYMENT_STARTUP]), 50);
    
                return false;
            }
        }
        else if (this.isClientPath())
        {
            this._logger.debug('No payment method found. Redirect to maintenance page.');

            setTimeout(() => this.goToMaintenance(), 50);
    ...
    resolveKinderConnectPath(): void
    {
        if (!_.isNull(this.getKinderConnectPath()) && !_.isNull(this._navService.getRouteObject(this.getKinderConnectPath())))
        {
            const routeObj = this._navService.getRouteObject(this.getKinderConnectPath());

            this.removeKinderConnectPath();

            setTimeout(() => this._router.navigate([routeObj.url], { state: { skipGuard: '0' }}), 100);
        }
    }
    ...
    setPermissions(perms: string[]): void
    {
        try
        {
            this._logger.debug('[permission object size]', CommonHelper.getSizeOf(perms, true));
    ...
    {
        // this._logger.debug('[permission belongs to]', belongsTo);
        // this._logger.debug('[permission list]', perms);
    ...
    async checkClientAccount(): Promise<any>
    {
        return new Promise((resolve, reject) => 
        {
            ...
            {
                this._logger.debug('[client information]');
                ...
                const _subscription = this.getClientInformation()
                    ...
                    .subscribe(
                        response => this._logger.debug('😀 get client information done. 🍺', response),
                        error =>
                        {
                            console.error(error);
